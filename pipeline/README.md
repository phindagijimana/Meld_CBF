...
Runs **MELD Graph** on a subject's T1w, then brings that subject's **CBF** map
into the **same voxel grid as MELD's `prediction.nii.gz`**, and reports CBF
inside the predicted lesion cluster(s).
...
provenance); the `meldcbf` **CLI** is a thin, ergonomic wrapper over it. All the
neuroimaging work runs inside the **MELD apptainer image** (FreeSurfer,
`mri_coreg`, the GNN, and the `nibabel`/`nilearn` stats), so the host only needs
Snakemake + a few Python libs.
...
# (or skip packaging and run `python meldcbf/cli.py …`)
```
...
> shares the same in-container scripts, but the Snakemake/CLI path above is the
...
`prepare`, so each subject automatically gets the correct, CBF-contemporaneous
T1w (and FLAIR from that session if present). Re-run the resolver if the cohort
changes:
